...
function Update() {
    ...
    gl.viewport(0.0, 0.0, textureWidth, textureHeight);
...
function splatA(x, y, dx, dy, color) {
    splatProgram.bind();
    gl.uniform1i(splatProgram.uniforms.uTarget, velocity.first[2]);
    gl.uniform1f(
        splatProgram.uniforms.aspectRatio,
    ...
    if (window.innerWidth < 854) {
        if (!splatATriggered) {
            for (var i = 0; i < 10; i++) {
                ...
                splatA(x, y, dx, dy, color);
            }

            splatATriggered = true;

            setTimeout(() => {
                splatATriggered = false;
            }, 500);
        }
    }
});
